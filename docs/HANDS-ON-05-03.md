...
## 📄 Parte 5.5: Arquivos Completos (referência)

> Os passos acima mostram só os **trechos** que mudam. Abaixo estão os dois arquivos **inteiros**, no estado final da aula. Pode comparar/copiar direto.

### `dags/ml_pipeline_dag.py` (completo)
...
> 💡 **Sobre os passos opcionais desta aula**:
> - O **`flaky_task`** (Passos 3–4) é só para *ver* o retry acontecendo. Depois da demonstração, remova-o da DAG — por isso ele **não** aparece no arquivo final acima.
> - O **`schedule` com cron** (Passo 8) e o **timezone com `pendulum`** (Passo 9) são variações de produção. No arquivo final fica `schedule=None` (disparo manual); para agendar, troque por `schedule="0 2 * * *"`.

### `dags/ml_pipeline/tasks/ml_tasks.py` (completo)
...
def flaky_task(**context):
    """Task de demonstração que falha 50% das vezes (usada só no Passo 3-4)."""
    if random.random() < 0.5:
        raise RuntimeError("Falha simulada!")
    print("✅ Sucesso!")
```
